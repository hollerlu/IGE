...
countryCounts <- dataFile %>%
  ...
  countryCounts <- countryCounts %>% summarise(TripCount = n(), ProgramNames = list(ProgramName))

  # Putting the TripCounts into worldMap. Needed to first change NAME to CountryName
  # so that the geographical data matches the dataset
  ...
  worldMap <- merge(worldMap, countryCounts, by = 'CountryName', all.x = TRUE)
